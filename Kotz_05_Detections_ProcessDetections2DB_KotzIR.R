...
wd <- "C:/skh"
processed_file <- "2019TestF5C_tinyYolo_ir_20190905_updatedIR.csv"
flight <- "fl05"
camera <- "C"
...
#RPostgreSQL::dbSendQuery(con, "DELETE FROM surv_test_kamera.tbl_detections_processed_ir")

# Import data and process
## PROCESSED DATA
processed_id <- RPostgreSQL::dbGetQuery(con, "SELECT max(id) FROM surv_test_kamera.tbl_detections_processed_ir")
...
# Import data to DB
RPostgreSQL::dbWriteTable(con, c("surv_test_kamera", "tbl_detections_processed_ir"), processed, append = TRUE, row.names = FALSE)
