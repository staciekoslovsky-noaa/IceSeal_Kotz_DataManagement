# Process polar_bear detection data to DB
...
wd <- "C:/Users/stacie.hardy/Work/Work/Projects/AS_Detections/Data/Kotz_2019/F06"
original_file <- "2019TestF6C_tinyYolo_eo_20191205_originalRGB.csv"
processed_file <- "2019TestF6C_tinyYolo_eo_20191205_updatedRGB.csv"
flight <- "fl06"
camera <- "C"
reviewer <- "GMB"
...
#RPostgreSQL::dbSendQuery(con, "DELETE FROM surv_polar_bear.tbl_detections_original_rgb")
#RPostgreSQL::dbSendQuery(con, "DELETE FROM surv_polar_bear.tbl_detections_processed_rgb")

# Import data and process
## ORIGINAL DATA
original_id <- RPostgreSQL::dbGetQuery(con, "SELECT max(id) FROM surv_polar_bear.tbl_detections_original_rgb")
...
## PROCESSED DATA
processed_id <- RPostgreSQL::dbGetQuery(con, "SELECT max(id) FROM surv_polar_bear.tbl_detections_processed_rgb")
...
# Import data to DB
RPostgreSQL::dbWriteTable(con, c("surv_polar_bear", "tbl_detections_original_rgb"), original, append = TRUE, row.names = FALSE)
RPostgreSQL::dbWriteTable(con, c("surv_polar_bear", "tbl_detections_processed_rgb"), processed, append = TRUE, row.names = FALSE)
