...
# Define variables
wd <- "C:/skh/Footprints/fl05_fov_shapefiles"
...
#RPostgreSQL::dbSendQuery(con, "DELETE FROM surv_test_kamera.geo_images_footprint")

# Read shapefiles
shps <- list.files(path = wd, pattern = "shp", full.names = TRUE)

for (i in 1:length(shps)) {
  next_id <- RPostgreSQL::dbGetQuery(con, "SELECT max(id) FROM surv_test_kamera.geo_images_footprint")
  ...
  # Write data to DB
  sf::st_write(shape, con, c("surv_test_kamera", "geo_images_footprint"), append = TRUE)
}
